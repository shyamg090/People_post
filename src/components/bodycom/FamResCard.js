      ...
      <div className='rounded-lg w-full h-[12rem] overflow-hidden'>
        <div className='w-full h-full bg-cover bg-center transition-transform duration-1000 hover:scale-110' style={{ backgroundImage: `url("${resChain.image}")` }}  >
          <div className='w-full h-full flex items-end bg-gradient-to-t from-black to-transparent rounded-lg'>
            <h1 className='m-5 w-full text-[1rem] text-white'>{resChain?.cuisine}</h1>
          </div>
        </div> 
      </div>

      <h1 className='w-full text-[18px] text-tight font-extrabold leading-3'>{resChain?.name}</h1>
      <h4 className='text-[.8rem] w-full'>{resChain?.tags.join(", ")}</h4>
      <div className='w-full flex justify-evenly items-center '>
        <h3 className='flex items-center justify-center text-[14px] gap-1 rounded-[10px]  text-brown p-2' > {resChain?.rating}  <IoIosStar className='w-3 h-4' /> </h3>
        <h4 className='flex items-center justify-center text-[14px] gap-1 rounded-[10px]  text-brown p-2'>{resChain?.prepTimeMinutes} min</h4>
        <h3 className=' flex items-center justify-center text-[14px] gap-1 rounded-[10px]  text-brown p-2 font-extrabold'>serving {resChain?.servings}</h3>
      </div>
